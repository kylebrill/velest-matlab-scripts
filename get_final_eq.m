function varargout = get_final_eq(outfilename,printflag)
%GET_FINAL_EQ Read .OUT file from VELEST simultaneous run for final EQ locs
%   GET_FINAL_EQ(OUTFILE,PRINTFLAG) Reads .OUT file for earthquake
%   locations from final iteration of VELEST simultaneous run and outputs a
%   locations_raw_out.dat file if PRINTFLAG is set to 1.
%
%   [LATITUDE, LONGITUDE, DEPTH] =
%   = GET_FINAL_EQ(OUTFILENAME,PRINTFLAG) Reads .OUT file for earthquake
...
%   outputs locations_raw_out.dat file if PRINTFLAG is set to 1.
%
% Example:
%   [latitude,longitude,depth] =
%   get_final_eq('fuegorandtrial01.OUT',1);
...
startRow=find(~cellfun(@isempty,strfind(outfile,'~~~ output final hypocenters ...')))+3;
endRow=find(~cellfun(@isempty,strfind(outfile,'~~~ output final station residuals ...')))-2;
eqinfo = outfile(startRow:endRow,1);
...
% mag=str2double(eqcolumns(:,8));
% no=str2double(eqcolumns(:,9));
% rms=str2double(eqcolumns(:,10));
% x=str2double(eqcolumns(:,11));
% y=str2double(eqcolumns(:,12));
% z=str2double(eqcolumns(:,13));
...
if printflag == 1
    ...
    for in=1:length(eventnum)
        if in==length(eventnum)
            fprintf(fid,'%4d %7.4f %7.4f  %5.2f',eventnum(in,1),latitude(in,1),longitude(in,1),depth(in,1));
        else
            fprintf(fid,'%4d %7.4f %7.4f  %5.2f\n',eventnum(in,1),latitude(in,1),longitude(in,1),depth(in,1));
        end
    end
    fclose(fid);
end