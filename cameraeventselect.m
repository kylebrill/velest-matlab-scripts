%
% Sort events from antelope database according to camera images in order to
% treat them as "blasts" from a VELEST standpoint. 
% 
% Take the events from Events_from_Time_Lapse.mat pull out common events to
% see what is left.

load /mnt/data/Google' Drive'/Research/Fuego2012/Events_from_Time-Lapse.mat

% run ('/opt/antelope/5.6/setup.m') %Initialize Antelope
...
pwcut=1; %pick weight cutoff, values greater than this will not be printed

%outfile name
noncnvfile='slop-noexplns.CNV';
closecnvfile='slop-explns.CNV';
...
%% Get close event times

for jnd=1:length(ot)
    tmp=abs(everything(:,1)-datenum(strtime(ot(jnd))));
    mintimes(jnd,1)=min(tmp);
end
close=find(mintimes<datenum(0,0,0,0,0,10));
notclose=find(mintimes>datenum(0,0,0,0,0,10));
...
% minpicks=9; % number of detecting stations needed 
...
for nevt=1:length(oridsc)
    if ooc(nevt)>0
        tmp=find(ooc(nevt)==ao);
        ...
        if cutter >= minpicks %select events with number of detections
            if inside(nevt) %select events within radius of a station
                if odc(nevt) < mindepth %select events within cone
...
% HPLMP0  4.31JALMP0  4.53
                    % convert from epoch time to string to datenum to 
                    % datevec
                    ...
                    [yr,mo,da,hr,mn,sc]=datevec(datenum(strtime(otc(nevt))));
                    odatenum=datenum(strtime(otc(nevt)));
                    fprintf(fid,...
                        '%02d%02d%02d %02d%02d %05.2f %7.4fN %8.4fW %7.2f   %4.2f  360  9.9 9.99  9.9  9.9\n',...
                        yr-2000,mo,da,hr,mn,sc,vollat,vollon,volele,magc(nevt));
                    acceptedex = acceptedex + 1; %Count number of events printed
...
fclose(fid);
display([num2str(acceptedex) ' events in close .CNV'])
%% nonexplosions
fid=fopen(noncnvfile,'w');
% a=find(oo>0);
acceptednon = 0;
%  oridstmp=orids(a)
orids=orids(notclose);
mag=mag(notclose);
nass=nass(notclose);
od=od(notclose);
ola=ola(notclose);
...
% HPLMP0  4.31JALMP0  4.53
                    ...
                    acceptednon = acceptednon + 1; %Count number of events printed
                    % write out the pick info
                    for npk=1:cutter
%                         wt=[]; diffsec=[];
                        % adjust pick time
...
fclose(fid);
display([num2str(acceptednon) ' events in nonexplosion .CNV'])